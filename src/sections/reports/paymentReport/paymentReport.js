...
import {GetPaymentReport,clearPaymentReport} from "../../../redux/reports/reports.actions"
import { connect } from 'react-redux'
import {Redirect, useHistory, withRouter } from 'react-router-dom'

const PaymentReport = ({fetchSaleListStart,getClients,clients,GetClientPricesStart,clientWithPrices,fetchTelecomStart,telcoList,paymentReport,GetPaymentReport,FetchCategoriesStart,categories,clearPaymentReport}) =>
{
    ...
    useEffect(()=>
    {
        if (withParams)
        {
            ...
            {
                GetPaymentReport(downloadParams)
            }
        }
    ...
    if(paymentReport)
    {
        let arr =  paymentReport
        let ar = arr[arr.length - 1]
        console.log(ar)
        if (ar)
        {
            paymentDue = ar.runningTotal + ar.amount
        }
        ...
        <div className="content">
         {reRoute.val ? <Redirect to={`/Reports/${reRoute.nav}`}/> : null}
            <div className="subHeader-reports">
                <h2 className="subText-reports">Reports/Payment Report</h2>
            ...
            <div className="reports-blackBar">
                <div className="blackBar-totalCost">Payment Due  {" "}  {paymentDue} </div>
            </div>
            <div className="reports-content">
                <div className='reports-titles' style={{width:"1500px"}} >
                    ...
                    <div className='reports-title'>Mode</div>
                    <div className='reports-title'>Description</div>
                </div>
                    {
                     paymentReport ? paymentReport.map((report,index)=>
                     {
                        return  <div className='reports-vals' style={{width:"1500px"}} key={index}>
                                    ...
                                    <div className='reports-title-val' style={{color:"#01AA62"}}>{report.amount}</div>
                                    <div className='reports-title-val'>{report.runningTotal + report.amount}</div>
                                    <div className='reports-title-val'>{report.paymentMode}</div>
...
const mapDispatchToProps = dispatch =>
{
    return{
        ...
        GetPaymentReport:(params)=>dispatch(GetPaymentReport(params)),
        clearPaymentReport:()=>dispatch(clearPaymentReport())
    }
}
...
const mapStateToProps = state =>
{
    return{
        ...
        paymentReport:state.reports.paymentReport
    }
}

export default withRouter(connect(mapStateToProps,mapDispatchToProps)(PaymentReport))